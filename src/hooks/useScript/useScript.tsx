...
// Dynamically load an external script in one line with this React hook to integrate third party scripts dynamically.
...
export const useScript = (src: string): Status => {
  ...
  useEffect(
    () => {
      ...
      const setStateFromEvent = (event: Event) => setStatus(event.type === 'load' ? Status.Ready : Status.Error);
