...
import { generateDefaultData } from '../StackedAreaChartWidget.helpers';
import { ReadOnlyStackedAreaChartWidget } from '../StackedAreaChartWidget';
...
// const YAXIS_LINE_SELECTOR = '.highcharts-axis.highcharts-yaxis .highcharts-axis-line'; // Reference
...
describe('widgets/StackedArea.tsx', () => {
  it('should render the widget', () => {
    renderWidget(<ReadOnlyStackedAreaChartWidget />, { widget: { ...mockWidget, widgetId: mockId } });
  ...
  it('should update x and y axis line colour', () => {
    ...
    const { container } = renderWidget(<ReadOnlyStackedAreaChartWidget />, { widget });
    expect(container.querySelectorAll(XAXIS_LINE_SELECTOR)[0].getAttribute('stroke')).toBe(newXAxisColor);

    // Comment out the one below now because we do not have the option to enable/disable the yAxis.
    // When the option is avaliable, we should test the case when it is enable, and another test for disable.
    // expect(container.querySelectorAll(YAXIS_LINE_SELECTOR)[0].getAttribute('stroke')).toBe(newYAxisColor);
  });
  ...
  it('axis titles should be bold by default', () => {
    const { container } = renderWidget(<ReadOnlyStackedAreaChartWidget />, {
