import { createImage, makeDataUrl, getMimeType, toArray } from './html-to-image.helpers';
...
async function decorate<T extends HTMLElement>(nativeNode: T, clonedNode: T): Promise<T> {
...
export async function cloneNode<T extends HTMLElement>(node: T, options: Options, isRoot?: boolean): Promise<T | null> {
  ...
  return (
    Promise.resolve(node)
      ...
      // Note: this can combine with cloneChildren if performance needs to optimize
      .then((clonedNode) => removeTextContent(clonedNode, options))
  );
}
...
/**
 * Remove text content from giving node
 *
 * @param node - A html element
 * @param options - parse options
 * @returns
 */
async function removeTextContent<T extends HTMLElement>(node: T, options: Options): Promise<T> {
  if (!options?.filterText?.(node)) return node;
